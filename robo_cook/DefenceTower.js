...
import Evil from "../prefabs/Evil.js";
...
export default class Game { 
  ...
  create() {
    ...
    // setup the pathfinding with  --EasyStar.js-- library
        ...
        this.evilFound = this.pathEvilFound.bind(this);
        this.easystar.findPath(4, 0, 0, 4, this.evilFound);
    // setup the pathfinding with  --EasyStar.js-- library
    ...
    this.nextEvilSpawn = this.game.time.now  + this.evilSpawnTime;
  ...
  update() {
        ...
        if(this.game.input.activePointer.isDown && this.selectedTile && (this.game.time.now > this.nextSoldierTime)) {
        	if(!this.selectedTile.occupant && this.selectedTile.buyable) {
	        	if(this.money >= 100) {
                    ...
                    //console.log("New Soldier created!");
	        	}
        ...
        if( this.game.time.now > this.nextEvilSpawn) {
        	this.spawnEvil();
            this.nextEvilSpawn = this.game.time.now + this.evilSpawnTime;        
        }
        else if(this.game.time.now > this.nextSpawn) {
        	this.spawnEnemy();
        	this.nextSpawn = this.game.time.now + this.spawnTime;
        }
  ...
  checkTiles(tile) {
        ...
        if (!tile.selected && inBounds) {
            tile.selected = true;
            tile.tint = 0x86bfda;
            this.game.add.tween(tile).to({ isoZ: 8 }, 200, Phaser.Easing.Quadratic.InOut, true);
  ...
  spawnTiles() {
        ...
        for (var y = 0; y <= map_height; y ++) {
            ...
            for (var x = 0; x <= map_width; x ++) {
                ...
                //this.game.add.isoSprite(x*size, y*size, 1, "water", 0)
                if(tileName=="roadEW")
                    tile = this.game.add.isoSprite(x*size, y*size, 1, tileName, 1, this.isoGroup);
                else
    ...
    pathEvilFound(path) {
	    if (path === null) {
	        ...
	        this.convertedEvilPath = [];
	        var curPoint;
	        for(var i = 0; i < path.length; i++) {
	        	curPoint = this.gameTiles[path[i].y][path[i].x];
	        	this.convertedEvilPath.push( { x: curPoint.isoX, y: curPoint.isoY } )
	        }
    ...
    spawnEvil() {
		var skel = new Evil(this.game, this.convertedEvilPath[0].x, this.convertedEvilPath[0].y);
        skel.setPath(this.convertedEvilPath);
        skel.advanceTile();
        skel.pathFinished.addOnce(this.enemyAtGoal, this);
        this.enemies.add(skel);
    }
